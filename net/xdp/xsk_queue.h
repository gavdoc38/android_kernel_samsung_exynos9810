					    ...
					    struct xdp_desc *desc)
{
	...
	if (likely(desc->len)) {
		end = desc->addr + desc->len - 1;
		chunk_end = xp_aligned_extract_addr(pool, end);
		if (chunk != chunk_end)
			return false;
	}
					    ...
					    struct xsk_buff_pool *pool, u32 max)
{
	...
	while (cached_cons != q->cached_prod && nb_entries < max) {
		struct xdp_rxtx_ring *ring = (struct xdp_rxtx_ring *)q->ring;
		struct xdp_desc *desc = &descs[nb_entries];
		u32 idx = cached_cons & q->ring_mask;

		*desc = ring->desc[idx];
		if (unlikely(!xskq_cons_is_valid_desc(q, desc, pool))) {
			/* Skip the entry */
			cached_cons++;
			continue;
		}
...
static inline u32 xskq_cons_peek_desc_batch(struct xsk_queue *q,
					    struct xdp_desc *descs,
					    struct xsk_buff_pool *pool,
					    u32 max)
{
...
/* To improve performance in the xskq_cons_release functions, only update
 * local state here. Reflect this to global state when we get new entries
 * from the ring and when Rx or Tx processing completes in the NAPI loop.
 */
...
static inline u32 xskq_prod_reserve_addr_batch(struct xsk_queue *q,
					       struct xdp_desc *descs,
					       u32 max)
{
	struct xdp_umem_ring *ring = (struct xdp_umem_ring *)q->ring;
