...
static void __xp_dma_unmap(struct xsk_dma_map *dma_map, unsigned long attrs)
{
	...
	for (i = 0; i < dma_map->dma_pages_cnt; i++) {
		...
		if (*dma) {
			dma_unmap_page(dma_map->dev, *dma, PAGE_SIZE,
				       DMA_BIDIRECTIONAL);
			*dma = 0;
		}
	}
...
static void xp_check_dma_contiguity(struct xsk_dma_map *dma_map)
{
	...
	for (i = 0; i < dma_map->dma_pages_cnt - 1; i++) {
		if (dma_map->dma_pages[i] + PAGE_SIZE ==
		    dma_map->dma_pages[i + 1])
...
static int xp_init_dma_info(struct xsk_buff_pool *pool,
			    struct xsk_dma_map *dma_map)
{
	size_t elem_size = sizeof(*pool->dma_pages);
	u32 count = dma_map->dma_pages_cnt;

	pool->dma_pages = kvcalloc(count, elem_size, GFP_KERNEL);
	if (!pool->dma_pages)
		return -ENOMEM;
	       ...
	       unsigned long attrs, struct page **pages, u32 nr_pages)
{
	...
	(void)attrs;
	for (i = 0; i < dma_map->dma_pages_cnt; i++) {
		dma = dma_map_page(dev, pages[i], 0, PAGE_SIZE,
				   DMA_BIDIRECTIONAL);
