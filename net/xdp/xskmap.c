...
static struct xsk_map_node *
xsk_map_node_alloc(struct xsk_map *map, struct xdp_sock __rcu **entry)
{
...
static struct bpf_map *xsk_map_alloc(union bpf_attr *attr)
{
	...
	err = bpf_map_precharge_memlock(round_up(size, PAGE_SIZE) >>
					PAGE_SHIFT);
	if (err)
		return ERR_PTR(err);
			       ...
			       u64 map_flags)
{
	...
	spin_lock_bh(&m->lock);
	old_xs = rcu_dereference_protected(*map_entry,
