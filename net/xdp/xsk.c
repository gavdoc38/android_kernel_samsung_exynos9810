...
u32 xsk_tx_peek_release_desc_batch(struct xsk_buff_pool *pool,
				   struct xdp_desc *descs,
				   u32 max_entries)
{
	struct xdp_sock *xs;
...
xsk_get_map_entry(struct xdp_sock *xs, struct xdp_sock __rcu ***map_entry)
{
...
static void xsk_delete_from_maps(struct xdp_sock *xs)
{
	...
	while ((map = xsk_get_map_entry(xs, &map_entry))) {
			  ...
			  char __user *optval, int __user *optlen)
{
	...
	case XDP_STATISTICS:
	{
		...
		if (extra_stats) {
			...
			stats.rx_fill_ring_empty_descs = xs->pool ?
				xskq_nb_queue_empty_descs(xs->pool->fq) : 0;
