...
static int env_resolve_init(struct btf_verifier_env *env)
{
	...
	resolved_sizes = kvzalloc(nr_types * sizeof(*resolved_sizes),
				  GFP_KERNEL | __GFP_NOWARN);
	if (!resolved_sizes)
		goto nomem;

	resolved_ids = kvzalloc(nr_types * sizeof(*resolved_ids),
				GFP_KERNEL | __GFP_NOWARN);
	if (!resolved_ids)
		goto nomem;

	visit_states = kvzalloc(nr_types * sizeof(*visit_states),
				GFP_KERNEL | __GFP_NOWARN);
	if (!visit_states)
		goto nomem;
				    ...
				    bool ptr_to_mem_ok)
{
	...
	if (!t || !btf_type_is_func(t)) {
		/* These checks were already done while loading function BTF
		 * or while collecting kernel function calls.
		 */
	...
	if (nargs > MAX_BPF_FUNC_REG_ARGS) {
		bpf_log(log, "Function %s has %d > %d args\n", func_name,
			nargs, MAX_BPF_FUNC_REG_ARGS);
		return -EINVAL;
	}

	/* Check that BTF function arguments match the verifier state. */
	for (i = 0; i < nargs; i++) {
		u32 regno = i + 1;
			  ...
			  struct bpf_reg_state *reg)
{
	...
	for (i = 0; i < nargs; i++) {
		...
		if (btf_type_is_ptr(t)) {
			...
			ref_t = btf_resolve_size(btf, t, &reg[i + 1].mem_size);
			if (IS_ERR(ref_t)) {
				bpf_log(log,
					"arg#%d reference type('%s %s') size cannot be determined: %ld\n",
					i, btf_type_str(t),
...
struct btf *btf_parse_vmlinux(void)
{
	...
	/* Find map structures used to validate map pointer accesses. */
		    ...
		    struct bpf_insn_access_aux *info)
{
	...
	arg = off / 8;
	args = (const struct btf_param *)(t + 1);
	/* If t is NULL, use the default register argument count. */
			   ...
			   struct btf_func_model *m)
{
	...
	if (!func) {
		 ...
		 * Fall back to register-sized u64 arguments.
		 */
...
/* Type IDs are meaningful only together with their BTF object. */
			  ...
			  const struct btf *need_btf, u32 need_type_id)
{
	...
	/* Are we already done? */
	if (off == 0 &&
	    btf_types_are_same(btf, id, need_btf, need_type_id))
		return true;
...
static ssize_t btf_module_read(struct file *file, struct kobject *kobj,
