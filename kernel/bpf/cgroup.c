...
static int bpf_cgroup_storages_alloc(
	struct bpf_cgroup_storage *storages[],
	struct bpf_cgroup_storage *new_storages[],
	enum bpf_attach_type type, struct bpf_prog *prog, struct cgroup *cgrp)
{
	enum bpf_cgroup_storage_type stype;
	...
	for_each_cgroup_storage_type(stype) {
		struct bpf_cgroup_storage_map *storage_map;
		...
		storage_map = (void *)map;
		storage = cgroup_storage_lookup(storage_map, &key, false);
		storages[stype] = storage;
		if (storage)
			continue;
			...
			enum bpf_attach_type type, u32 flags)
{
	...
	struct bpf_cgroup_storage
		*new_storage[MAX_BPF_CGROUP_STORAGE_TYPE] = {};
	struct bpf_prog_list *pl;
 ...
 *	otherwise
 * @ppos: value-result argument: value is the current file position, result is
 *	the position set by the BPF program or the initial value if unchanged
				     ...
				     struct bpf_prog *prog, u32 *target_size)
{
	...
	case offsetof(struct bpf_sysctl, file_pos):
		if (type == BPF_WRITE) {
			...
			*insn++ = BPF_STX_MEM(BPF_SIZEOF(u32), treg,
						  si->src_reg, 0);
			*insn++ = BPF_LDX_MEM(
...
{
	return net_gen_cookie(ctx ? sock_net(ctx->sk) : &init_net);
}
