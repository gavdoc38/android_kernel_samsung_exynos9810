...
/*
 * The BPF dispatcher is a multiway branch code generator. It replaces an
 * expensive indirect program call with a direct branch when the architecture
 * supplies both dispatcher code generation and BPF text-poking support.
 */
...
{
	s64 ips[BPF_DISPATCHER_MAX] = {}, *ipsp = ips;
	int i;
	...
	return arch_prepare_bpf_dispatcher(image, ips, d->num_progs);
}
				  ...
				  int prev_num_progs)
{
	...
	if (new && bpf_dispatcher_prepare(d, new))
		return;
