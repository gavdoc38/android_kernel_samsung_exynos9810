...
/* Find a module section: 0 means not found. Ignore SHF_ALLOC. */
...
/* Find a section while ignoring SHF_ALLOC and count its objects. */
...
static int find_module_sections(struct module *mod, struct load_info *info)
{
	...
	mod->btf_data = any_section_objs(info, ".BTF", 1,
					 &mod->btf_data_size);
#endif
...
#ifdef CONFIG_DEBUG_INFO_BTF_MODULES
	/* .BTF is not SHF_ALLOC and will be removed; sanitize its pointer. */
	mod->btf_data = NULL;
#endif
	/*
...
static int prepare_coming_module(struct module *mod)
{
	...
	err = blocking_notifier_call_chain_robust(
		&module_notify_list, MODULE_STATE_COMING,
		MODULE_STATE_GOING, mod);
...
static int modules_open(struct inode *inode, struct file *file)
{
	...
	if (!err) {
		...
		m->private = kallsyms_show_value(file->f_cred) ?
			NULL : (void *)8ul;
	}
