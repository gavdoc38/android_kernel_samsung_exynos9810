 ...
 *	@redirected: packet was reinjected after a redirect
...
static inline bool
skb_flow_dissect_flow_keys_buf(const struct net *net, struct flow_keys *flow,
			       void *data, __be16 proto, int nhoff,
			       int hlen, unsigned int flags)
{
	memset(flow, 0, sizeof(*flow));
	return __skb_flow_dissect(net, NULL, &flow_keys_buf_dissector,
				  flow, data, proto, nhoff, hlen, flags);
}
