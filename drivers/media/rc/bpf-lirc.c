...
// bpf-lirc.c - handles BPF programs attached to raw IR devices
//
	   ...
	   u32, toggle)
{
	...
	ctrl = container_of(sample, struct ir_raw_event_ctrl, bpf_sample);
	rc_keydown(ctrl->dev, (enum rc_type)protocol, (u32)scancode,
		   toggle != 0);
...
/* Called after the raw IR thread has stopped with the handler lock held. */
...
int lirc_prog_query(const union bpf_attr *attr, union bpf_attr __user *uattr)
{
	...
	if (attr->query.prog_cnt && prog_ids && cnt)
